...
/*
** Flags for the open(Port, File, FLAG) call
*/
#define  UDS_TCP_CLIENT             0
#define  UDS_TCP_SERVER             1
#define  UDS_UDP_CLIENT             2
#define  UDS_UDP_SERVER             3
#define  UDS_UDP_CLIENT_CONNECT     4
...
my_open(char *path, int is_server)
{
    ...
    if ((s = socket(PF_LOCAL, is_server == UDS_UDP_SERVER || is_server == UDS_UDP_CLIENT ? SOCK_DGRAM : SOCK_STREAM, 0)) < 0) {
	    return -1;
    }
    ...
    switch (is_server) {
        case UDS_TCP_CLIENT: res = connect(s, (struct sockaddr *) &sin, sizeof(sin)); break;
        case UDS_UDP_SERVER:
        case UDS_TCP_SERVER: res = bind   (s, (struct sockaddr *) &sin, sizeof(sin)); break;
        case UDS_UDP_CLIENT: res = 0; break;
        case UDS_UDP_CLIENT_CONNECT: res = connect(s, (struct sockaddr *) &sin, sizeof(sin)); break;
        default: res = -2;
    }
    return (res < 0) ? res : s;
}
