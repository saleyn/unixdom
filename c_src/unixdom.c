...
static void s1_output(ErlDrvData drv_data, char *buf, ErlDrvSizeT len);
static ErlDrvSSizeT s1_control(ErlDrvData drv_data, unsigned int command, char *buf, ErlDrvSizeT len,
...
static void invoke_s1_closefd(void *data);
...
static void
s1_output(ErlDrvData drv_data, char *buf, ErlDrvSizeT len)
{
    /*
...
s1_outputv(ErlDrvData drv_data, ErlIOVec *ev)
{
    ...
    case S1_GETFD:
        ...
        break;
    case S1_CLOSEFD:
        c->invoke = invoke_s1_closefd;
        EV_GET_UINT32(ev, &c->i.fd, &p, &q);
